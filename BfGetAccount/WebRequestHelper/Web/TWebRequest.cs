...
namespace WebRequestHelper
{
    ...
    public class TWebRequest
    {
        ...
        protected TWebRequest(string fileName):this()
        {
        ...
        {
            HttpWebResponse response = GetResponse(url, postData, method);
        ...
        {
            string dataOuput = data.Aggregate("", (current, keyValuePair) => current + string.Format("{0}={1}", keyValuePair.Key, keyValuePair.Value));
